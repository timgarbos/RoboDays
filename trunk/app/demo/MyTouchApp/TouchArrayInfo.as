package app.demo.MyTouchApp{
    ...
    public class TouchArrayInfo extends Sprite {

		private var my_line:MovieClip = new MovieClip();
		
        public function TouchArrayInfo():void {
		...
		public function updateCall():void{  
			...
			if(TUIO.OBJECT_ARRAY.length >= 2){
				for (var i=0; i<TUIO.OBJECT_ARRAY.length; i++) {
					addChild(my_line);
					my_line.graphics.lineStyle(10, 0xF46000);  
					my_line.graphics.moveTo(TUIO.OBJECT_ARRAY[i].x,TUIO.OBJECT_ARRAY[i].y);  
					if(TUIO.OBJECT_ARRAY[i+1] != undefined){
						my_line.graphics.lineTo(TUIO.OBJECT_ARRAY[i+1].x,TUIO.OBJECT_ARRAY[i+1].y);  
						//Line.graphics.lineTo(blobs[i+1].e.stageX,blobs[i+1].e.stageY);
					}else{
						my_line.graphics.lineTo(TUIO.OBJECT_ARRAY[0].x,TUIO.OBJECT_ARRAY[0].y);  
						//Line.graphics.lineTo(blobs[0].e.stageX,blobs[0].e.stageY);
					}
				}
			}
			else if(blobs.length == 1){ // Makes new Start Point for lines
				my_line.graphics.moveTo(TUIO.OBJECT_ARRAY[0].x,TUIO.OBJECT_ARRAY[0].y)
			}
			else{ // Clears Lines
			}
        }
