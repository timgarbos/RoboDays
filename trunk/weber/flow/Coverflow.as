...
package weber.flow{
	...
	public class Coverflow extends Sprite {
		...
		////////////////////////////////////////////
		public function Coverflow(_width:Number, _height:Number, __stage:Stage = null,parentMc:MovieClip = null):void {
			_stage=__stage;
			_parentmc = parentMc;
			sw=_width;
			sh=_height;
		...
		private function coverflowXMLLoader_Complete(e:Event):void {
			...
			for (var i=0; i<coverflowItemsTotal; i++) {
				...
				_obj.dataIndex = i;
				_obj.description = (coverflowXML.cover[i].@description.toString());
				_obj.votes = (int)(coverflowXML.cover[i].@votes.toString());
				_data[i] = _obj;
		...
		private function loadCover():void {
			...
			coverSlider.x = (_parentmc.width/2) - (coverSlider.width/2);
			coverSlider.y=_parentmc.height-40;
			...
			coverLabel.x = (_parentmc.width/2) - (coverLabel.width/2);
		...
		private function cover_Opened(event:CoverflowItemEvent):void {
			dispatchEvent(event);
		}
		private function cover_Selected(event:CoverflowItemEvent):void {
			...
			if (coverArray[currentCover].rotationY==0) {
				try {
					// Dispatch event if center cover is clicked!
					
					dispatchEvent(new CoverflowItemEvent(CoverflowItemEvent.COVERFLOWITEM_OPENED, event.data));
		...
		private function createCover(num:uint, url:String):Sprite {
			...
			cover.addEventListener(CoverflowItemEvent.COVERFLOWITEM_OPENED, cover_Opened);
